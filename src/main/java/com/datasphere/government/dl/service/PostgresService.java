...
public class PostgresService extends BaseService {
    ...
    private final static String PG_PRONAME = "SELECT pg_proc.proname,pg_type.typname,pg_proc.pronargs FROM pg_proc JOIN pg_type " +
    ...
    private final static String PG_PROC = "SELECT prosrc,proargtypes,proargnames FROM pg_catalog.pg_proc where proname = ?;";
//1043 20 {ids,userid}
    /**
     * 取得数据连接
     * @return
    ...
    public Connection getConnection() {
        ...
        try {
            ...
            conn.setSchema(this.schemaNamePG);
        } catch (Exception e) {
    ...
    public List<String> listAllStoredProcedure() {
        ...
        try {
            conn = getConnection();
            pst = conn.prepareStatement(PG_PRONAME);
            rs = pst.executeQuery();
            while (rs.next()) {
    ...
    public Map<String, String> listStoredProcedureByName(String proname) {
        ...
        try {
            conn = getConnection();
            pst = conn.prepareStatement(PG_PROC);
