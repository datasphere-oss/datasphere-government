create or replace function proc_ht_fundnav()
returns void as
$$
declare
  --v_code VARCHAR2(20);
  ...
  v_etime     timestamp;
  v_btime     timestamp;
  v_errortext varchar(2000);
  v_errorcode varchar(2000);
  v_pkgname   varchar(2000);
  v_pkgsname  varchar(2000);
           ...
           (select end_tradingday
              from (select tradingday,
                      ...
                      from (select tradingday, to_char(tradingday - 1, 'd') rk
                              from center_admin.pub_tradingday
                             where istradingday = 1
                               and exchangecode = 101) as hashdata) as hashdata1
     ...
     where t2.nav_cycle = 3 and t2.nav_flag = 1
       ...
       and t1.d_biz in (select tradingday
                          from (select tradingday,
                                  ...
                                  from center_admin.pub_tradingday
                                 where istradingday = 1
                                   and exchangecode = 101) as hashdata2
                         where rk = 1);
